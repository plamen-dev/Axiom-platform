...
- Revit 2026 (lane-3A operator target): `.\scripts\deploy-revit-2026.ps1 -BuildOnly` then without `-BuildOnly` to deploy (`Axiom.Revit.2026.sln`, SDK-style `net8.0-windows`, `REVIT_2026`; deploys to `C:\ProgramData\Autodesk\Revit\Addins\2026\`). Build-verified only until the first lane-3A run — record deviations in the multi-version runbook.
...
- `RevitElementIdCompat` uses `ElementId.Value` under `REVIT_2026 || REVIT_2027`; 2024 keeps `IntegerValue`. Add new version symbols to the compat helper, not scattered conditionals.
