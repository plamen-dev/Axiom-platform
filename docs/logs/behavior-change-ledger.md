...
## BHV-032: Evidence-quality substance gate quarantines semantically empty evidence (Finding 2)

| Field | Value |
|-------|-------|
| **behavior_id** | BHV-032 |
| **date** | 2026-06-23 |
| **capability** | Evidence promotion / execution-chain evidence writer (promotion-scoring / evidence-quality infrastructure, not a Revit capability) |
| **observed_prompt** | `python -m poetry run axiom execution-chain-run --json-output` on Windows produced `evidence.json` with `status=PASS` but `module_count=0`; `capability-evidence-apply` then moved confidence `very_low -> very_high` and readiness `blocked -> ready` |
| **previous_behavior** | `EvidencePromotionLoop.apply()` derived accept/reject purely from the id-flow / plumbing outcome (`bundle.passed` / `bundle.status` / sibling `trace.status`). A chain run whose self-model produced zero modules still yielded `status=PASS`, so an empty-substance bundle was **accepted** and promoted to `very_high` confidence / `ready` readiness in a single application. No verdict distinguished substantive evidence from semantically empty evidence. |
| **expected_behavior** | The id-flow `status` stays a plumbing verdict, but a separate evidence-quality / substance verdict must mark self-model evidence whose required key metrics are all zero as `EMPTY`, and promotion must quarantine such evidence (no confidence / readiness / trust / promotion mutation) while keeping it inspectable. Capabilities without a configured rule must be `NOT_EVALUATED` and preserve current behavior; all-zero metrics must not be globally invalid. |
| **current_behavior** | A shared helper `axiom_core.evidence_quality.evaluate_quality(capability_id, metrics)` returns a `quality` verdict `{verdict: SUBSTANTIVE|EMPTY|NOT_EVALUATED, required_metrics, zero_metrics, reason}`. A small per-capability table `_REQUIRED_METRICS = {"self-model-build": ("module_count",)}` drives the rule; `self-model-build` is `EMPTY` iff `module_count` is missing or `<= 0` (`import_edge_count` / `isolated_module_count` are reported in the reason but not required). The execution-chain evidence writer (`execution_chain_orchestrator._write_evidence`) stamps `quality` onto `evidence.json` so the verdict travels with the bundle; the chain's id-flow `status` is unchanged. `EvidencePromotionLoop.apply()` inserts a quality gate after the identity/conflict/stale gates and before duplicate/accept/score accumulation: an `EMPTY` verdict is `QUARANTINED` via the existing `EvidenceDecision.QUARANTINED` + intake store (no new outcome family), with no confidence/readiness/trust mutation, and the intake record persists `evidence_quality` for inspection. Promotion **defensively recomputes** the verdict from the bundle's `metrics` when the `quality` field is absent or malformed, so older/hand-authored empty bundles cannot bypass the gate. Confidence math, readiness projection, execution-chain ID-flow, and the accept/fail/duplicate/stale/conflict paths are otherwise unchanged. |
| **status** | implemented (Python; `tests/test_evidence_quality.py` 14 passed + `tests/test_evidence_promotion.py` / `tests/test_execution_chain_orchestrator.py` 53 passed + ruff green on Ubuntu). True on-Windows re-run pending operator (lane-2 not yet validated). |
| **related_bug_id** | Finding 2 (Windows lane-2: semantically empty evidence accepted too aggressively) |
| **related_test_case** | `tests/test_evidence_quality.py` (rule + table + resolve/recompute); `tests/test_evidence_promotion.py` (empty quarantined, no very_low->very_high, no blocked->ready, substantive still promotes, unknown-capability NOT_EVALUATED accepted, older/malformed bundle recomputed+quarantined, producer stamps EMPTY while status PASS, end-to-end empty-repo quarantine) |
| **related_artifact_path** | `src/axiom_core/evidence_quality.py` (new), `src/axiom_core/execution_chain_orchestrator.py` (producer stamp), `src/axiom_core/evidence_promotion.py` (consumer gate) |

## BHV-033: Local Runner Windows-safe invocation uses direct venv module execution (PR #52 rework)

| Field | Value |
|-------|-------|
| **behavior_id** | BHV-033 |
| **date** | 2026-06-23 |
| **capability** | Local Runner command invocation (infrastructure tooling, not a Revit capability) |
| **observed_prompt** | Local Runner `ruff` / `execution_chain_run` on `C:\Dev\Axiom\Code\Axiom-platform` (Windows, inside Poetry venv) |
| **previous_behavior** | PR #52's `_windows_safe_command` rewrote a leading `poetry ...` to `[sys.executable, "-m", "poetry", ...]` and inner `poetry run ruff/pytest` to `... run python -m ruff/pytest`. On Windows the runner executes inside Poetry's managed venv, whose interpreter lacks the `poetry` module, so the rewrite self-defeated with `No module named poetry`; the bare `.exe` shim path had already failed with WDAC `WinError 4551`. Net: Local Runner tool actions did not run on Windows. |
| **expected_behavior** | Avoid recursive Poetry entirely; invoke project-installed tools directly with the current venv interpreter, and invoke Axiom via a real module entrypoint (not the `.exe` shim and not a nonexistent `axiom_core.cmd`). Keep the allowlist as the command boundary; no new action names, task schema, or arbitrary command execution. No-op on Linux/CI. |
| **current_behavior** | On Windows only, `_windows_safe_command` rewrites `poetry run <tool> ...` heads via `_WINDOWS_TOOL_MODULE`: `ruff` → `[<python>, -m, ruff, ...]`, `pytest` → `[<python>, -m, pytest, ...]`, `axiom` → `[<python>, -m, axiom_cli, ...]`. The `poetry` wrapper is dropped; no `poetry` token, `.exe` shim, or `<python> -m poetry` recursion survives. `axiom_cli` is a new module entrypoint (`src/axiom_cli/__main__.py`) that calls the same `cli` callable as the `axiom` console script (`axiom_cli.main:cli`). Non-poetry heads (`git`, `dotnet`, `powershell`) are untouched. Off Windows the transform is a no-op, so Linux/CI keep `poetry run ...`. Allowlist, task schema, workspace policy, and action names unchanged. |
| **status** | implemented (Python; `tests/test_local_runner.py` `TestWindowsSafeCommand` + `TestAxiomCliModuleEntrypoint`; ruff green on Ubuntu). True on-Windows re-run pending operator (lane-2 not yet validated). |
| **related_bug_id** | BUG-020 (recursive `python -m poetry` from PR #52) |
| **related_test_case** | `tests/test_local_runner.py` (`test_poetry_run_ruff_rewritten`, `test_poetry_run_pytest_rewritten`, `test_poetry_run_axiom_rewritten_to_axiom_cli`, `test_poetry_run_axiom_evidence_apply_rewritten`, `test_no_recursive_poetry_emitted`, `test_no_shim_or_poetry_survives_any_affected_action`, `test_execute_task_applies_rewrite_via_injected_windows_flag`, `test_main_module_exposes_console_script_callable`, `test_python_m_axiom_cli_help_works`, `test_python_m_axiom_cli_lists_execution_chain_run`) |
| **related_artifact_path** | `tools/local_runner/local_runner.py` (`_windows_safe_command`, `_WINDOWS_TOOL_MODULE`), `src/axiom_cli/__main__.py` (new), `docs/runbooks/local-runner-runbook.md` (Windows-safe invocation section) |
| **notes** | Supersedes the PR #52 rewrite rules, which assumed `python -m poetry` was reachable from the runner's interpreter. Poetry is intentionally **not** installed into the project venv. No promotion/confidence/readiness/evidence semantics touched; no Revit behavior; 2024 baseline unaffected. |
| **notes** | Reuses existing quarantine mechanism + intake store — no new evidence framework, no new outcome family, no confidence-scoring redesign, no readiness redesign, no execution-chain ID-flow change, no implementation-worker / retry / GPR / Revit behavior. Empty evidence is quarantined and inspectable, not discarded. Zero metrics are enforced only per capability rule, never globally. 2024 Revit baseline unaffected; no Revit live validation required. Separate PR from the Windows-safe runner invocation (#52) and the inventory path fix (#53). |

---

## BHV-034: Local Runner emits a tracked evidence summary (attested → captured)

| Field | Value |
|-------|-------|
| **behavior_id** | BHV-034 |
| **date** | 2026-06-23 |
| **capability** | Local Runner evidence-return path (infrastructure tooling, not a Revit capability) |
| **observed_prompt** | Operator wants a durable, committable proof object for a loop run (chain → evidence → promotion) instead of gitignored scratch |
| **previous_behavior** | The loop's outputs (`artifacts/execution_chain/`, `artifacts/capability_evidence_intake/`, `artifacts/capability_confidence/`, `artifacts/local_runner_runs/`) are gitignored, so a run's evidence quality and promotion decision were *attested* (present locally) but not *captured* (committable/reviewable). No runner action produced a tracked summary. |
| **expected_behavior** | A new allowlisted, read-only Local Runner action emits a compact, tracked summary of the newest chain bundle and its promotion outcome under `artifacts/validation_runs/<summary_id>/`, with no state mutation and no arbitrary task-supplied paths. |
| **current_behavior** | New allowlisted action `emit_evidence_summary` runs in-process (no subprocess). It resolves the newest `artifacts/execution_chain/<run>/evidence.json` itself (runner-derived, sandbox-validated — no task-supplied path), reads the bundle's `quality` + `references` and the run's `trace.json` (id-flow `status`), matches the newest intake `report.json` by `chain_run_id`/`evidence_id`, and writes `evidence_summary.json` + `evidence_summary.md` under `artifacts/validation_runs/<summary_id>/`. Contents: `run_id`, `capability_id`, chain id-flow `status`, evidence `quality_verdict`, promotion `decision` (or `not_applied`), current confidence/readiness, before→after where available, **relative** source artifact paths, git commit, UTC timestamp. It never mutates confidence/readiness/trust/promotion/capability state and never records raw stdout, secrets, or absolute/machine-specific paths. If no bundle exists it returns `status: blocked` with guidance to run `execution_chain_run` first. `artifacts/validation_runs/<summary_id>/` is not gitignored (only `vrun_*/` is), so the operator commits the summary as captured proof. |
| **status** | implemented (Python; `tests/test_evidence_summary.py`; ruff green on Ubuntu). Windows lane-2 re-run pending operator (lane-2 not yet validated). |
| **related_bug_id** | (none — additive capability, PR B / variant A) |
