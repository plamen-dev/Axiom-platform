...
| **notes** | Read-only inspector only — no new knowledge framework, no new evidence system, no canonical mutation, no committed generated artifacts, no implementation-worker / retry / GPR / Revit behavior. Not canonical truth by itself; it is a context preflight / live system map consumed by Program 1, Devin, and future PR preflight. System Atlas component families are hardcoded from the PR #157 design pass + old-foundation scan; file presence is checked live. Tracked docs are committed; generated artifacts are gitignored. The required old-foundation scan reconciled older pipe/spine/bridge/job-plan/MCP/agents concepts: results recorded in the Duplicate/Alias Map (Clusters 10–12 + "Old-foundation scan result" section) and the PR Purpose Map old-foundation table. Mapping only — no pipe/spine/bridge/orchestrator/runtime behavior changed. |
