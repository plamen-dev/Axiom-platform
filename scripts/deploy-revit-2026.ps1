<#
.SYNOPSIS
    Build and deploy Axiom add-in to Revit 2026.

.DESCRIPTION
    Builds the Axiom.Revit.2026 solution in Release|x64, then copies the
    output DLLs and .addin manifest to the Revit 2026 add-ins folder.
...
.EXAMPLE
    .\deploy-revit-2026.ps1
    .\deploy-revit-2026.ps1 -Configuration Debug
    .\deploy-revit-2026.ps1 -BuildOnly
#>
param(
...
$slnPath = Join-Path $repoRoot "src\axiom_revit\Axiom.Revit.2026.sln"
$addinSource = Join-Path $repoRoot "src\axiom_revit\Axiom.RevitAddin.2026.addin"
$addinTarget = "C:\ProgramData\Autodesk\Revit\Addins\2026"
$outputDir = Join-Path $repoRoot "src\axiom_revit\Axiom.RevitAddin.2026\bin\x64\$Configuration\net8.0-windows"

Write-Host "=== Axiom Revit 2026 Build & Deploy ===" -ForegroundColor Cyan
Write-Host ""
...
# --- Step 2: Verify Revit 2026 API DLLs ---
Write-Host "[2/4] Checking Revit 2026 API..." -ForegroundColor Yellow
$revitApiPath = "$env:ProgramW6432\Autodesk\Revit 2026\RevitAPI.dll"
$revitApiUiPath = "$env:ProgramW6432\Autodesk\Revit 2026\RevitAPIUI.dll"
...
Write-Host "Restart Revit 2026 to load the add-in." -ForegroundColor White
