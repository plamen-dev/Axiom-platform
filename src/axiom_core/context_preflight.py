...
_COMPONENT_FAMILIES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "purpose": "Core Pydantic data models for the earlier-introduced job→plan→execution→QA pipeline.",
        ...
        "notes": "Earlier-introduced pipeline; coexists with newer execution-chain models.",
    },
    {
        "name": "Orchestrator (job→plan→MCP)",
        ...
        "related_prs": [],
        "status": "active (earlier-introduced pipeline)",
    ...
    {
        "name": "MCP Layer (tool protocol boundary)",
        "aliases": ["MCPLayer", "MCP", "MCP layer", "ToolDefinition", "ToolExecutionContext"],
        "primary_files": ["src/axiom_core/mcp_layer.py"],
        "purpose": "Tool protocol boundary for Revit operations. Currently a mock that simulates Revit tool execution; real impl connects to the Revit add-in.",
        "workflow_edge": "Orchestrator/Plan → MCPLayer.execute_tool → ToolResult (simulated)",
        "related_prs": [],
        "status": "active (mock); real Revit-connected impl not proven in repo (partial)",
        "overlap_risk": "high — overlaps with Automation Bridge / Pipe transport as the 'Revit execution boundary' (see Cluster 10)",
        "notes": "Earlier-introduced foundational component. Self-described as mock. Distinct responsibility (protocol/tool catalog) from the named-pipe transport. Adapter seam between mock MCP and real pipe bridge is unresolved.",
    },
    {
        "name": "Automation Bridge / Pipe Transport (Python side)",
        "aliases": [
            "AutomationBridge", "automation bridge", "PipeClient", "pipe client",
            "named pipe", "axiom pipe",
        ],
        "primary_files": [
            "src/axiom_core/automation_bridge.py",
            "src/axiom_core/pipe_client.py",
        ],
        "purpose": "Python→Revit transport: PipeClient sends JSON-RPC execute_tool over named pipe to in-Revit AxiomPipeServer; AutomationBridge adds non-interactive driver + durable evidence.",
        "workflow_edge": "Capability request → PipeClient (named pipe) → AxiomPipeServer (in Revit) → ToolResult + bridge evidence",
        "related_prs": ["PR #2 (original named-pipe bridge, per code docstring)"],
        "status": "active; Windows-local revalidation pending (post-PR #151). Falls back to mock off-Windows.",
        "overlap_risk": "high — this is the REAL Revit execution path vs MCPLayer's mock (adapter seam needed)",
        "notes": "Earlier-introduced foundational component. AutomationBridge v0 (repo-local PR #19) reuses existing PipeClient (repo-local PR #2). This is the actual autonomy-loop transport edge; do not reinvent under a new 'task packet consumer' or 'local worker' name without reconciling here.",
    },
    {
        "name": "Revit Add-in Bridge (C#: PromptDispatcher / AxiomPipeServer)",
        "aliases": [
            "PromptDispatcher", "prompt dispatcher", "AxiomPipeServer",
            "Axiom.RevitAddin", "PromptCommand", "ToolRegistry (C#)",
        ],
        "primary_files": [
            "src/axiom_revit/Axiom.Core/Bridge/PromptDispatcher.cs",
            "src/axiom_revit/Axiom.Core/Bridge/AxiomPipeServer.cs",
            "src/axiom_revit/Axiom.RevitAddin/PromptCommand.cs",
            "src/axiom_revit/Axiom.RevitAddin/App.cs",
        ],
        "purpose": "In-Revit C# side: AxiomPipeServer hosts the named pipe; PromptDispatcher routes prompt text to registered C# capabilities (C# equivalent of Python prompt_resolver).",
        "workflow_edge": "Named pipe request → AxiomPipeServer → PromptDispatcher/ToolRegistry → Revit capability (main thread via ExternalEvent)",
        "related_prs": [],
        "status": "active (C#); covered by Revit-version compatibility rules (2024 baseline / 2027 isolated). Python pytest does not exercise it.",
        "overlap_risk": "medium — PromptDispatcher (C#) parallels prompt_resolver (Python); distinct runtimes (adapter seam by design)",
        "notes": "Earlier-introduced foundational component. Not a duplicate of Python prompt_resolver — it is the in-Revit counterpart. Do not unify across the language boundary.",
    },
    {
        "name": "Run Spine (audit/evidence backbone)",
        "aliases": ["run_spine", "run spine", "RunMetadata", "AuditEntry", "Run Spine"],
        "primary_files": ["src/axiom_core/run_spine.py"],
        "purpose": "Mandatory execution wrapper: every action gets a run ID, standard artifact folder, structured audit logs, machine-readable result files.",
        "workflow_edge": "Cross-cutting: any capability/runner action → run folder + audit log + artifact manifest",
        "related_prs": [],
        "status": "active",
        "overlap_risk": "medium — 'run spine' artifact folder vs execution-chain Evidence/Report stages vs evidence bundles (see Cluster 11)",
        "notes": "Earlier-introduced backbone (repo-local PR #31; docstring: 'the backbone that all future capabilities must use'). Distinct responsibility (run identity + audit folder) from execution-chain evidence stages, but they both produce durable artifacts — reconcile before changing evidence/validation bundle layout.",
    },
    {
        "name": "Automation Planner (event-driven planning)",
        "aliases": ["AutomationPlanner", "automation_planner", "execution lane", "policy gate"],
        "primary_files": ["src/axiom_core/automation_planner.py"],
        "purpose": "Accepts project/model/change events, decides which capabilities may need to run, classifies execution lanes, generates a dry-run plan. Does NOT mutate models.",
        "workflow_edge": "Change event → AutomationPlanner → automation_plan.json + policy_gate.json (dry-run only)",
        "related_prs": [],
        "status": "active (planning/recommendation only; no autonomous dispatch from it)",
        "overlap_risk": "medium — 'plan' overlaps with Orchestrator Plan and ExecutionPlan; this is event→lane planning, not step planning",
        "notes": "Earlier-introduced foundational component (repo-local PR #35). Built on Run Spine. Distinct responsibility (event-driven lane planning) from Orchestrator (job→step planning) and ExecutionPlan (chain stage).",
    },
    {
        "name": "Persistence / ExecutionTrace Store",
        "aliases": ["Storage", "persistence", "ExecutionTrace", "ExecutionTraceRow", "SQLite job store"],
        "primary_files": ["src/axiom_core/persistence.py"],
        "purpose": "SQLAlchemy/SQLite storage for jobs, plans, and ExecutionTrace records of the earlier-introduced pipeline.",
        "workflow_edge": "Orchestrator/MCP execution → ExecutionTrace → Storage.save_trace (queryable by job_id)",
        "related_prs": [],
        "status": "active (earlier-introduced pipeline persistence)",
        "overlap_risk": "high — ExecutionTrace vs ExecutionReport vs evidence bundles vs run-spine artifacts all record 'what happened' (see Cluster 11)",
        "notes": "Earlier-introduced foundational component. ExecutionTrace is the earlier-introduced execution record; execution-chain ExecutionReport (PR #142) is the newer one. Both currently active — duplicate-candidate for the 'execution record' concept; reconcile before adding a new validation/evidence bundle model. Not classified legacy: neither is proven retired.",
    },
    {
        "name": "Agents Layer (Orchestrator/Execution/Telemetry)",
        "aliases": ["OrchestratorAgent", "ExecutionAgent", "TelemetryAgent", "agents"],
        "primary_files": [
            "src/axiom_core/agents/orchestrator_agent.py",
            "src/axiom_core/agents/execution_agent.py",
            "src/axiom_core/agents/telemetry_agent.py",
        ],
        "purpose": "Vertical-slice coordination: OrchestratorAgent resolves prompt → delegates to ExecutionAgent → logs via TelemetryAgent. Agents coordinate; capabilities execute.",
        "workflow_edge": "Prompt → OrchestratorAgent → resolve_prompt → ExecutionAgent (capability) → TelemetryAgent",
        "related_prs": [],
        "status": "active (vertical slice); coordination only",
        "overlap_risk": "high — OrchestratorAgent vs Orchestrator vs ExecutionChainOrchestrator vs runners are all 'coordinators' (see Cluster 2)",
        "notes": "Earlier-introduced foundational component. Per architecture rule: agents coordinate, do not own capability logic. Distinct responsibility from the data-model Orchestrator (orchestrator.py).",
    },
    {
