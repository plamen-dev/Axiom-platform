...
_COMPONENT_FAMILIES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "notes": "Old-foundation. Self-described as mock. Distinct responsibility (protocol/tool catalog) from the named-pipe transport. Adapter seam between mock MCP and real pipe bridge is unresolved.",
    },
    {
        ...
        "notes": "Old-foundation. AutomationBridge v0 explicitly reuses existing PipeClient (PR #2). This is the actual autonomy-loop transport edge; do not reinvent under a new 'task packet consumer' or 'local worker' name without reconciling here.",
    },
    {
        ...
        "notes": "Old-foundation. Not a duplicate of Python prompt_resolver — it is the in-Revit counterpart. Do not unify across the language boundary.",
    },
    {
        ...
        "notes": "Old-foundation backbone (docstring: 'the backbone that all future capabilities must use'). Distinct responsibility (run identity + audit folder) from execution-chain evidence stages, but they both produce durable artifacts — reconcile before changing evidence/validation bundle layout.",
    },
    {
        ...
        "notes": "Old-foundation. Built on Run Spine. Distinct responsibility (event-driven lane planning) from Orchestrator (job→step planning) and ExecutionPlan (chain stage).",
    },
    {
        ...
        "purpose": "SQLAlchemy/SQLite storage for jobs, plans, and ExecutionTrace records of the legacy pipeline.",
        "workflow_edge": "Orchestrator/MCP execution → ExecutionTrace → Storage.save_trace (queryable by job_id)",
        "related_prs": [],
        "status": "active (legacy pipeline persistence)",
        ...
        "notes": "Old-foundation. ExecutionTrace is the legacy execution record; execution-chain ExecutionReport (PR #142) is the newer one. Duplicate-candidate for the 'execution record' concept — reconcile before adding a new validation/evidence bundle model.",
    },
    {
        ...
        "notes": "Old-foundation. Per architecture rule: agents coordinate, do not own capability logic. Distinct responsibility from the data-model Orchestrator (orchestrator.py).",
    },
    {
