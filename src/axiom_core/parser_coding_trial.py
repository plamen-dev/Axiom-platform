"""Parser Coding Trial v1 — controlled practical test of the verification stack.

Runs a minimal, deterministic parser coding trial that validates
parse_key_value_lines through existing testing and evidence conventions.

Non-goals: no workflow engine, no autonomous repair loop, no scheduler,
no agent assignment, no architecture changes, no broad parser framework.
"""
...
class ParserCodingTrialRunner:
    """Runs a minimal parser coding trial and generates evidence."""
    ...
    def __init__(
        ...
        self._trials_dir = self._artifacts_root / "parser_coding_trials"
    ...
    def run_trial(
        ...
        function_name: str = "parse_key_value_lines",
        description: str = "",
    ) -> dict[str, Any]:
        ...
        trial = {
            ...
            "description": description or f"Parser coding trial for {function_name}",
    ...
    def _write_evidence(
        ...
        (evidence_dir / "parser_coding_trial_request.json").write_text(
            json.dumps(request_data, indent=2, default=str),
            encoding="utf-8",
        )

        (evidence_dir / "parser_coding_trial_result.json").write_text(
            json.dumps(trial, indent=2, default=str),
            encoding="utf-8",
        )

        md = self._generate_summary(trial)
        (evidence_dir / "parser_coding_trial_summary.md").write_text(
            md, encoding="utf-8",
        )
    ...
    def _generate_summary(trial: dict[str, Any]) -> str:
        ...
        lines: list[str] = []
        lines.append(f"# Parser Coding Trial: {trial['function_name']}")
        lines.append("")
        ...
        if trial["escalation_needed"]:
            ...
            lines.append(
                "No escalation or repair objects needed — validation passed."
            )
        lines.append("")
